#!/bin/bash
# Requisitos tener instalado el paquerte (en fedora) inotify-tools o su similar en ubuntu
# Uso:
#     continous-test.sh [--env[=<path-activate-virtualenv>]] [--src=<path-to-source>] [--test=<path-to-test>]
#         [--prefix=<test-prefix>] [--postfix=<test-postfix> [--filter=<test-filter>]
